        ...
        {STEPS.map((s, i) => (
          <span key={s.n} className="flex items-baseline gap-3 md:gap-4">
            <span className="flex items-baseline gap-2">
              <span className="pixel-sans step-num text-fg-40 text-xs md:text-sm">{s.n}</span>
              <span className={`pixel-serif text-fg ${large ? 'text-2xl md:text-4xl' : 'text-xl md:text-3xl'}`}>{s.title}</span>
            </span>
            {i < STEPS.length - 1 && (
              <span className={`text-fg-25 ${large ? 'text-xl md:text-3xl' : 'text-lg md:text-2xl'}`}>→</span>
            )}
          </span>
        ))}
      </div>
      <p className="pixel-sans text-fg-60 text-sm md:text-base leading-relaxed max-w-3xl mx-auto text-center mt-8 md:mt-10">
        {LIFECYCLE_SUMMARY}
      </p>
    </div>
  );
