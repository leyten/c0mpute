...
// Plan definitions — the network runs ONE public model, so there is exactly
// one entry (the picker collapses to it; no tier names anywhere). `workerModel`
// matches the orchestrator MODEL_CATALOG so per-model worker counts line up.
// `vision`/`thinking` gate the composer controls.
export const PLANS = [
