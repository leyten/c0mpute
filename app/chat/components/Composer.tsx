...
// The message composer: autosizing textarea, model picker, deep-thinking
    ...
    <div className="relative px-4 pb-4 md:pb-5">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!hasWorkers && otherCount === 0 && isConnected && (
          <div className="pixel-sans text-white/50 text-xs text-center mb-2">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl focus-within:border-white/25 transition-colors">
          <textarea
            ...
            disabled={disabled}
            className="w-full bg-transparent px-4 pt-3.5 pb-1.5 pixel-sans text-white text-base placeholder:text-white/35 focus:outline-none resize-none overflow-y-auto disabled:opacity-50"
          />
          <div className="flex items-center gap-1.5 px-2.5 pb-2.5">
            ...
            {selectedPlanObj.vision && (
              <button
                ...
                className="cursor-pointer p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/[0.06] transition-colors disabled:opacity-20 disabled:cursor-not-allowed"
                title="Upload image"
            ...
            {inputValue.length > MAX_INPUT_CHARS * 0.75 && (
              <span className={`pixel-sans text-xs mr-1 tabular-nums ${inputValue.length > MAX_INPUT_CHARS * 0.9 ? 'text-red-400' : 'text-white/50'}`}>
            ...
            <button
              ...
              className="cursor-pointer w-9 h-9 rounded-full bg-white text-black hover:bg-white/90 flex items-center justify-center transition-all disabled:opacity-25 disabled:cursor-not-allowed shrink-0"
              aria-label="Send"
            >
