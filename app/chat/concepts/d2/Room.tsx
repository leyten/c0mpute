...
// A focused conversation — one sheet on the desk. Opening one is entering a
// place: the top bar names it and holds its desk controls (tags, pin, archive,
// delete, and on wide screens "open beside"). In split view this same room
// renders in either pane; the active pane carries a steel accent, and closing a
// pane is one step back. The stream renders through the same markdown path as
...
import type { Convo, LiveJob, Prompt } from './store';
import MessageView, { ImageSkeleton } from './MessageView';
import Composer from './Composer';
import { TagChip, TagEditor } from './Tags';
import { IconArchive, IconChevronDown, IconChevronLeft, IconColumns, IconPin, IconTag, IconTrash } from './Icons';
...
export default function Room({
  engine, convo, live, error, canRetry, pendingImageMsgId, anonBlocked, prompts,
  split, active, canSplit,
  getDraft, onDraftChange, onFocus, onClose, onSplit,
  onRename, onTogglePin, onToggleArchive, onDelete, onSend, onRetry, onStop, onDismissError,
  onSelectModel, onToggleThink, onAddTag, onRemoveTag, onSavePrompt, onDeletePrompt,
}: {
  engine: ChatEngine;
  convo: Convo;
  live: LiveJob | null;
  ...
  /** Two panes are open. */
  split: boolean;
  /** This is the pane keyboard actions target (only meaningful when split). */
  active: boolean;
  /** Wide screen and a single pane open: "open beside" is available. */
  canSplit: boolean;
  getDraft: () => string;
  onDraftChange: (t: string) => void;
  onFocus: () => void;
  /** Back one level: single view leaves to the desk, split closes this pane. */
  onClose: () => void;
  onSplit: () => void;
  ...
  // Tags edit inline in a slim strip under the header, toggled from the header.
  ...
  const hasTags = convo.tags.length > 0;

  return (
    <div className="flex flex-col h-full min-h-0" onMouseDown={onFocus}>
      {split && (
        <div className={`h-0.5 shrink-0 transition-colors ${active ? 'bg-[#80a0c1]/70' : 'bg-transparent'}`} />
      )}
      <header className={`border-b border-white/10 shrink-0 transition-opacity ${split && !active ? 'opacity-60' : ''}`}>
        <div className="max-w-5xl mx-auto px-3 md:px-6 h-14 flex items-center gap-2 md:gap-3">
          <button
            onClick={onClose}
            ...
            title={split ? 'close this pane (esc)' : 'back to the desk (esc)'}
          >
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            <button
              ...
              title="tags"
              className={`cursor-pointer p-2 rounded-lg hover:bg-white/[0.06] transition-colors ${tagsOpen || hasTags ? 'text-[#80a0c1]' : 'text-white/40 hover:text-white'}`}
            >
            ...
            {canSplit && (
              <button
                onClick={onSplit}
                title="open another conversation beside this one"
                className="cursor-pointer p-2 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06] transition-colors"
              >
        ...
        {(tagsOpen || hasTags) && (
          <div className="max-w-5xl mx-auto px-3 md:px-6 pb-2 -mt-0.5 flex items-center gap-2">
            <span className="pixel-sans text-[9px] uppercase tracking-[0.18em] text-white/25 shrink-0">tags</span>
            {tagsOpen ? (
              <TagEditor tags={convo.tags} onAdd={onAddTag} onRemove={onRemoveTag} autoFocus />
            ) : (
              <div className="flex flex-wrap items-center gap-1.5">
                {convo.tags.map(t => <TagChip key={t} label={t} />)}
                <button
                  onClick={() => setTagsOpen(true)}
                  title="edit tags"
                  className="cursor-pointer pixel-sans text-[9px] uppercase tracking-[0.16em] text-white/30 hover:text-white/60 transition-colors"
                >
                  edit
                </button>
              </div>
