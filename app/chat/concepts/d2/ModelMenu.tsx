...
export default function ModelMenu({ engine, current, onSelect }: {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="cursor-pointer flex items-center gap-2 rounded-full border border-white/10 hover:border-white/25 px-3 py-1.5 pixel-sans text-[12px] text-white/70 hover:text-white transition-colors"
        title="choose a model"
      >
        <span className={`w-1.5 h-1.5 rounded-full ${workers > 0 ? 'bg-[rgba(52,211,153,0.9)]' : 'bg-white/25'}`} />
        {plan.name}
        <IconChevronDown className={`w-3 h-3 text-white/40 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="absolute bottom-full left-0 mb-2 z-50 w-[19rem] rounded-xl border border-white/10 bg-[#161311] shadow-2xl shadow-black/60 p-1.5">
            {engine.models.map(m => {
              ...
              return (
                <button
                  ...
                  className={`cursor-pointer w-full text-left px-3 py-2.5 rounded-lg transition-colors ${active ? 'bg-white/[0.06]' : 'hover:bg-white/[0.04]'}`}
                >
