...
// out as editorial cards with subject, model, recency, chalk tags, and a
// one-line tail. Search is instant and prominent, a tag row filters the desk to
// one label at a time, pinned work sits first, archived work stays reachable
// below. An empty desk is a deliberate state, not a gap.
...
import { allTags, searchDesk, type Convo } from './store';
import { TagChip, TagEditor } from './Tags';
import { IconArchive, IconChevronDown, IconDots, IconPin, IconPlus, IconSearch, IconTag, IconTrash, IconX } from './Icons';
...
export default function Library({ engine, convos, liveConvoId, onOpen, onCreate, onTogglePin, onToggleArchive, onDelete, onRename, onAddTag, onRemoveTag }: {
  engine: ChatEngine;
  convos: Convo[];
  ...
  const q = query.trim();
  // The tag filter narrows the whole desk to one label before search or sort.
  // Derive the effective filter so a tag that was just removed everywhere stops
  // filtering on its own, with no follow-up state to reconcile.
  const tags = useMemo(() => allTags(convos), [convos]);
  const filterTag = activeTag && tags.includes(activeTag) ? activeTag : null;
  const scoped = useMemo(
    () => (filterTag ? convos.filter(c => c.tags.includes(filterTag)) : convos),
    [convos, filterTag],
  );
  const byRecency = (a: Convo, b: Convo) => b.updatedAt.localeCompare(a.updatedAt);
  const pinned = useMemo(() => scoped.filter(c => !c.archived && c.pinned).sort(byRecency), [scoped]);
  const active = useMemo(() => scoped.filter(c => !c.archived && !c.pinned).sort(byRecency), [scoped]);
  const archived = useMemo(() => scoped.filter(c => c.archived).sort(byRecency), [scoped]);
  const hits = useMemo(() => searchDesk(scoped, q), [scoped, q]);
  ...
  const cardMenu = (c: Convo) => (
    <div
      className="absolute top-10 right-3 z-30 w-56 rounded-lg border border-white/10 bg-[#161311] py-1 shadow-2xl shadow-black/60"
    ...
    >
      ...
      <div className="px-3 py-1.5">
        <p className="pixel-sans text-[9px] uppercase tracking-[0.18em] text-white/30 mb-1.5 flex items-center gap-1.5">
          <IconTag className="w-3 h-3" /> tags
        </p>
        <TagEditor tags={c.tags} onAdd={t => onAddTag(c.id, t)} onRemove={t => onRemoveTag(c.id, t)} />
      </div>
      <div className="border-t border-white/10 my-1" />
      <button
        onClick={() => {
        ...
        <div className="flex flex-wrap gap-1.5">
          {c.tags.map(t => (
            <TagChip
              ...
              label={t}
              active={filterTag === t}
              title={filterTag === t ? 'clear filter' : `filter by ${t}`}
          ...
          {total === 0 ? (
            <div className="py-16 md:py-28 text-center">
              ...
              <p className="pixel-sans text-sm md:text-base text-white/45 mt-4 max-w-md mx-auto leading-relaxed">
                Conversations you start live here in full view: subject, model, tags, and where you left off.
            ...
            <>
              <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
                <div>
                  ...
                  <p className="pixel-sans text-[10px] uppercase tracking-[0.16em] text-white/35 mt-2.5">
                    {pinned.length + active.length} {filterTag ? `tagged ${filterTag}` : 'on the desk'}
              ...
              <div className="flex items-center gap-3 border-b border-white/15 focus-within:border-white/40 transition-colors pb-3 mb-6">
              ...
              {tags.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap mb-10">
                  <span className="pixel-sans text-[9px] uppercase tracking-[0.18em] text-white/25 mr-1">tags</span>
                  {tags.map(t => (
                    <TagChip
                      key={t}
                      label={t}
                  ...
                  {filterTag && (
                    <button
                      ...
                      className="cursor-pointer pixel-sans text-[10px] uppercase tracking-[0.14em] text-white/40 hover:text-white transition-colors ml-1"
                    >
                      clear
                    </button>
                  )}
                    ...
                    <div>
                      {hits.map(({ convo: c, snippet }) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap mt-1.5 pixel-sans text-[10px] uppercase tracking-[0.14em] text-white/30">
                            <span>{planName(c.model)} · {formatChatDate(c.updatedAt)}{c.archived ? ' · archived' : ''}{c.pinned ? ' · pinned' : ''}</span>
                ...
                <>
                  ...
                  {pinned.length + active.length === 0 && (
                    <p className="pixel-sans text-sm text-white/40 mb-10">
                      {filterTag
                        ? `Nothing tagged ${filterTag} is on the desk. Clear the tag to see everything.`
