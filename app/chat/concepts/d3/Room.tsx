...
export default function Room({ engine, convo, live, error, canRetry, pendingImageMsgId, anonBlocked, paletteOpen, modKey, onOpenPalette, getDraft, onDraftChange, onBack, onRename, onTogglePin, onToggleArchive, onDelete, onSend, onRetry, onStop, onDismissError, onSelectModel, onToggleThink }: {
  engine: ChatEngine;
  convo: Convo;
  live: LiveJob | null;
  error: string | null;
  ...
  // The room owns the composer's textarea ref so a printable key struck
  // anywhere in the room lands in the composer.
  ...
  // Keyboard flow. Escape leaves the room (the rename input keeps its own
  // Escape). A printable key struck while nothing is focused lands in the
  // composer, so you can just start typing. The palette owns the keyboard
  // while it is open.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (!e.metaKey && !e.ctrlKey && !e.altKey && e.key.length === 1 && !typing) {
      ...
      <header className="border-b border-white/10 shrink-0">
        <div className="max-w-5xl mx-auto px-3 md:px-6 h-14 flex items-center gap-2 md:gap-4">
          ...
          <button
            ...
            className="hidden sm:flex items-center pixel-sans text-[10px] text-white/30 hover:text-white/70 border border-white/10 hover:border-white/25 rounded px-1.5 py-1 transition-colors shrink-0"
          >
            {modKey}K
          </button>
