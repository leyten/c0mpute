...
//
// D3 adds a keyboard lane over the calm desk: arrows move a soft focus across
// the cards, Enter opens, `/` jumps to search, `n` starts a conversation, and
// Space opens a one-line quick-ask on the focused card. Quick-ask answers into
// the conversation in the background, without entering the room.
...
import { IconArchive, IconArrowUp, IconChevronDown, IconDots, IconPin, IconPlus, IconSearch, IconTrash, IconX } from './Icons';
...
export default function Library({ engine, convos, liveConvoId, paletteOpen, modKey, onOpenPalette, onOpen, onCreate, onQuickAsk, onTogglePin, onToggleArchive, onDelete, onRename }: {
  engine: ChatEngine;
  convos: Convo[];
  ...
  onQuickAsk: (id: string, text: string) => boolean;
  ...
  // Keyboard lane: a soft focus over the navigable cards (tracked by id so a
  // stale focus self-heals when the list changes), and the id of the card
  // whose inline quick-ask bar is open.
  ...
  // The list arrow keys move over: search results while searching, otherwise
  // pinned then the rest of the desk.
  const navList = useMemo(() => (q ? hits.map(h => h.convo) : [...pinned, ...active]), [q, hits, pinned, active]);

  const openAsk = (id: string) => { setAskId(id); setAskValue(''); setAskError(null); };
  const closeAsk = () => { setAskId(null); setAskValue(''); setAskError(null); };
  const submitAsk = (id: string) => {
    const v = askValue.trim();
    if (!v) return;
    if (onQuickAsk(id, v)) {
      closeAsk();
    } else {
      const gated = !engine.isAuthenticated &&
        (engine.anonCapReached || (engine.anonRemaining !== null && engine.anonRemaining <= 0));
      setAskError(
        gated ? 'sign in to keep asking'
          : !engine.connected ? 'connecting to the network'
            : 'the network is writing elsewhere — one job runs at a time',
      );
    }
  };

  // Desk-wide keys: '/' to search, `n` for a new conversation, arrows to move
  // the soft focus, Enter to open, Space to quick-ask the focused card. The
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (typing) return;
      if (e.key === 'n') { e.preventDefault(); onCreate(); return; }
      if (navList.length === 0) return;
      const cur = kbFocusId ? navList.findIndex(c => c.id === kbFocusId) : -1;
      if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
        e.preventDefault();
        setKbFocusId(navList[cur < 0 ? 0 : Math.min(cur + 1, navList.length - 1)].id);
      } else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
        e.preventDefault();
        setKbFocusId(navList[cur <= 0 ? 0 : cur - 1].id);
      } else if (e.key === 'Enter') {
        if (cur >= 0) { e.preventDefault(); onOpen(navList[cur].id); }
      } else if (e.key === ' ') {
        if (!q && cur >= 0) { e.preventDefault(); setAskId(navList[cur].id); setAskValue(''); setAskError(null); }
      }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [paletteOpen, navList, kbFocusId, q, onCreate, onOpen]);

  // Follow the soft focus into view. No-op when the focused id is not in the
  // current view, so a stale focus never scrolls.
  useEffect(() => {
    if (kbFocusId) document.getElementById(`d3-card-${kbFocusId}`)?.scrollIntoView({ block: 'nearest' });
  }, [kbFocusId]);
  ...
  const askBar = (c: Convo) => (
    <div onClick={e => e.stopPropagation()} className="mt-1 border-t border-white/10 pt-3">
      <div className="flex items-center gap-2 border border-white/15 focus-within:border-white/30 rounded-lg bg-white/[0.03] px-2.5 py-1.5 transition-colors">
        <input
          autoFocus
          value={askValue}
          onChange={e => { setAskValue(e.target.value); setAskError(null); }}
          onKeyDown={e => {
            if (e.key === 'Enter') { e.preventDefault(); submitAsk(c.id); }
            else if (e.key === 'Escape') { e.preventDefault(); closeAsk(); }
          }}
          placeholder={`ask ${planName(c.model)}…`}
          className="flex-1 min-w-0 bg-transparent outline-none pixel-sans text-[13px] text-white/90 placeholder:text-white/30"
        />
        <button
          onClick={() => submitAsk(c.id)}
          title="ask"
          className="cursor-pointer w-6 h-6 rounded-full bg-white text-black hover:bg-white/90 flex items-center justify-center shrink-0 transition-colors"
        >
          <IconArrowUp className="w-3 h-3" />
        </button>
      </div>
      <p className={`pixel-sans text-[10px] mt-1.5 ${askError ? 'text-red-300/90' : 'text-white/30'}`}>
        {askError ?? 'enter to ask · esc to close · the answer streams into this card'}
      </p>
    </div>
  );

  const card = (c: Convo) => {
    const kbFocused = kbFocusId === c.id;
    return (
      <article
        key={c.id}
        id={`d3-card-${c.id}`}
        onClick={() => { if (editingId !== c.id && askId !== c.id) onOpen(c.id); }}
        className={`group relative cursor-pointer border rounded-xl transition-colors p-5 flex flex-col gap-3 min-h-[10.5rem] ${
          kbFocused ? 'border-white/40 bg-white/[0.05] ring-1 ring-white/15' : 'border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/25'
        }`}
      >
        ...
        <div className="flex items-center gap-3 pixel-sans text-[10px] uppercase tracking-[0.14em] text-white/30">
          <span>{c.messages.length} {c.messages.length === 1 ? 'message' : 'messages'}</span>
          {liveConvoId === c.id && (
          ...
          {askId !== c.id && (
            <button
              onClick={e => { e.stopPropagation(); openAsk(c.id); }}
              title="ask without opening the conversation"
              className={`cursor-pointer pixel-sans text-[10px] uppercase tracking-[0.14em] text-white/30 hover:text-white transition-opacity ${
                kbFocused ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus:opacity-100'
              }`}
            >
              ask{kbFocused ? ' · space' : ''}
            </button>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-14 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <button
              ...
              title="commands and jump"
              className="hidden sm:flex items-center pixel-sans text-[10px] text-white/30 hover:text-white/70 border border-white/10 hover:border-white/25 rounded px-1.5 py-1 transition-colors"
            >
              {modKey}K
            </button>
            {engine.live ? (
      ...
      <main className="flex-1 overflow-y-auto" onClick={() => { setMenuId(null); setConfirmId(null); setAskId(null); }}>
            ...
            <>
              <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-2">
                  <kbd className="hidden md:block pixel-sans text-[10px] text-white/25 border border-white/10 rounded px-1.5 py-0.5" title="press n to start a conversation">n</kbd>
                  <button
                    onClick={() => onCreate()}
              ...
              <div className="flex items-center gap-3 border-b border-white/15 focus-within:border-white/40 transition-colors pb-3 mb-10">
                ...
                <input
                  ...
                  onKeyDown={e => {
                    ...
                    if (e.key === 'ArrowDown' && hits.length > 0) { e.preventDefault(); searchRef.current?.blur(); setKbFocusId(hits[0].convo.id); }
                    else if (e.key === 'Enter' && hits.length > 0) {
                      e.preventDefault();
                      const pick = (kbFocusId && hits.find(h => h.convo.id === kbFocusId)?.convo) || hits[0].convo;
                      onOpen(pick.id);
                    }
                  }}
                      ...
                      {hits.map(({ convo: c, snippet }) => {
                        ...
                        return (
                          <div
                            ...
                            id={`d3-card-${c.id}`}
                            onClick={() => onOpen(c.id)}
                            className={`cursor-pointer py-4 px-2 -mx-2 rounded-lg border-b border-white/[0.07] transition-colors ${kbFocused ? 'bg-white/[0.06] ring-1 ring-white/15' : 'hover:bg-white/[0.03]'}`}
                          >
